...
# Головний скрипт
    ...
    # Чекаємо до 60 секунд, чи ffmpeg завершиться. Якщо ні — припиняємо вручну.
    if timeout 60s bash -c "wait $ffmpeg_pid"; then
        echo "✅ ffmpeg завершився коректно"
    else
        echo "⚠️ ffmpeg завис — вбиваємо PID $ffmpeg_pid"
...
# Запускаємо всі створені скрипти з директорії
